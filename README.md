...
First, we setup the PWM-based MHC-I presentation predictor. 

Important: running ``MHC-I_rank_peptides.py`` only generates the PWMs for the MHC-I predictor.
It does not fix the MHC-I genotype used in the fine-tuning process.
This is specified in the hyper-parameter file used to call ``cape-mpnn.py`` (parameter ``mhc_1_alleles``).
We only tested ``b69bb1`` - using a single PWM for the specific whole genotype which is generated by the command below.
